-- Query 16: Tình hình chăm sóc của các CVTS đang như thế nào?
...
lead_stats AS (
    SELECT 
        ...
        COUNT(*) FILTER (WHERE l.stage = 'raw_lead') as raw_lead,
        COUNT(*) FILTER (WHERE l.stage = 'interested_in_major') as interested_in_major,
        COUNT(*) FILTER (WHERE l.stage = 'scholarship_applied') as scholarship_applied,
        COUNT(*) FILTER (WHERE l.stage = 'scholarship_deposited') as scholarship_deposited,
        COUNT(*) FILTER (WHERE l.stage = 'account_created') as account_created,
        COUNT(*) FILTER (WHERE l.stage = 'online_admission') as online_admission,
        COUNT(*) FILTER (WHERE l.stage = 'hardcopy_submitted') as hardcopy_submitted,
        COUNT(*) FILTER (WHERE l.stage = 'admitted') as admitted,
        COUNT(*) FILTER (WHERE l.stage = 'enrolled') as enrolled,
        COUNT(*) FILTER (WHERE l.stage = 'tuition_paid') as tuition_paid,
        COUNT(*) FILTER (WHERE l.stage = 'enrollment_cancelled') as enrollment_cancelled,
...
SELECT 
    ...
    -- Leads
    COALESCE(ls.total_leads, 0) as "Tổng leads",
    COALESCE(ls.raw_lead, 0) as "Raw Lead",
    COALESCE(ls.interested_in_major, 0) as "Quan tâm ngành",
    COALESCE(ls.scholarship_applied, 0) as "Đã nộp HB",
    COALESCE(ls.enrolled, 0) as "Nhập học"
...
LEFT JOIN lead_stats ls ON u.id = ls.user_id
