-- Query #22: Tình hình tuyển sinh tháng qua có gì?
...
-- 1. Thống kê Zalo conversations (30 ngày qua)
zalo_stats AS (
    SELECT 
        u.id as user_id,
        u.full_name,
...
-- 3. Thống kê VoIP calls (30 ngày qua)
voip_stats AS (
    SELECT 
        u.id as user_id,
    ...
    LEFT JOIN voip_call_records v ON em.ext = v.extension 
        AND v.createtime >= NOW() - INTERVAL '30 days'
    WHERE u.role IN ('cvts')
        AND u.status = 'active'
    GROUP BY u.id
),
-- 4. Thống kê Tasks (30 ngày qua)
task_stats AS (
    SELECT 
        u.id as user_id,
