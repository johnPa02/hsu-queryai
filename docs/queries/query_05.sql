-- Query 5: Thời gian nộp hồ sơ của các lead xét học bổng là khi nào?
...
SELECT 
    ...
    COALESCE(lsh.changed_at, l.updated_at) as "Thời gian đăng ký học bổng",
...
LEFT JOIN LATERAL (
    ...
    WHERE lead_id = l.id
        AND to_stage = 'scholarship_applied'
    ORDER BY changed_at DESC
    LIMIT 1
) lsh ON true

WHERE l.is_scholarship_applied = true
    AND l.status = 'active'
